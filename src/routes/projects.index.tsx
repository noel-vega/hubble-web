      ...
      <div className="p-6">
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${hasRunningContainers
                ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20"
                : "bg-slate-500/10 text-slate-700 dark:text-slate-400 border border-slate-500/20"
                }`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full ${hasRunningContainers ? "bg-emerald-500" : "bg-slate-400"
                  }`}
              />
...
function RouteComponent() {
  ...
  const stats = useMemo(() => {
    return {
      total: data.count,
      running: data.projects.reduce(
        (sum, p) => sum + p.containers_running,
        0,
      ),
      stopped: data.projects.reduce(
        (sum, p) => sum + p.containers_stopped,
        0,
      ),
